...
void FrameLessView::setcontext(AbstractAlgorithm* algorithm)
{
    //具体实例化哪个指标 expose qml
    switch (GlobalObject::g_grainType) {
      case GrainType::WHEAT:
        rootContext()->setContextProperty("algorithmResult",qobject_cast<AlgorithmWheat*>(algorithm));
        break;
      case GrainType::RICE:
    default:break;
    }
...
void FrameLessView::setSqlcontext(SqlSampleId * sql)
{
    switch (GlobalObject::g_grainType) {
      case GrainType::WHEAT:
        rootContext()->setContextProperty("sqlSampleId",qobject_cast<SqlWheatSampleId*>(sql));
        break;
      case GrainType::RICE:
    default:break;
    }
